...
- Protocol : 송신자, 수신자, 중간 장치가 따라야 하는 규칙
- 통신이 단순하면 -> Single-Layer Protocol충분
- 통신이 복잡하면 -> Protocol Layering 필요 

### Two Priciples of Protocl Layering
- 각 계층이 서로 반대되는 두 가지 작업을 수행
   - Listen <-> Talk
   - Encrpt <-> Decrpyt
   - Send Mail <-> Receive Mail

### Logical Connections
- 계층 간 논리적(가상) 연결이 존재한다고 생각
---
...
- 계층형 프로토콜은 **상호작용하는 모듈들(interactive modules)**로 구성되어 있으며, 각 모듈은 **특정 기능(specific functionality)**을 제공
- **특징** :  
...
- Layer 5 : **Application**
   - 프로세스 간 통신 담당(사용자 관점)
- Layer 4 : **Transport**
   - 프로세스 간 통신과 신뢰성 있는 데이터 전달을 담당(네트워크 관점)
- Layer 3 : **Network**
   - end to end 전송과 경로 결정
- Layer 2 : **Data link**
   - 프레임 단위의 안정적인 전송을 관리
- Layer 1 : **Physical**
   - 데이터를 실제 신호로 변환해 전달하는 계층
